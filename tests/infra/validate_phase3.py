...
async def validate_phase3():
    ...
    try:
        ...
        print(f"  ✅ Handshake successful. Negotiated TLS version: {result.tls_version or 'Unknown'}")
        ...
        for cert in certs:
            ...
            print(f"      - [{cert.cert_level.name}] {cert.public_key_algorithm} ({cert.signature_algorithm}) -> {safe_flag}")
    ...
    for asset in aggregated:
        cert_info = f" (Has {len(asset.certificate_chain_pem)} certs)" if asset.certificate_chain_pem else ""
        print(f"      - {asset.hostname or '<IP Only>'} ({asset.ip_address}:{asset.port}/{asset.protocol}) -> {asset.service_type.name}{cert_info}")
