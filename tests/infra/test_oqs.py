...
def test_oqs_kem_mechanisms_available() -> None:
    ...
    assert ml_kem_present, (
        f"ML-KEM / Kyber not found in enabled KEMs: {kems[:10]}..."
    )
...
def test_ml_kem_keygen() -> None:
    ...
    assert shared_secret_enc == shared_secret_dec, (
        "ML-KEM-768 encapsulation/decapsulation mismatch"
    )
...
def test_oqs_sig_mechanisms_available() -> None:
    ...
    assert ml_dsa_present, (
        f"ML-DSA / Dilithium not found in enabled sigs: {sigs[:10]}..."
    )
